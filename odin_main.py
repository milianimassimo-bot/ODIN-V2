...
# =============== STRATEGY MAPPING CENTRALIZZATO ===============
# Mapping centralizzato (fonte-verità unica)
TREND_STRATS = {
    "EMA_Trend_D1", "Keltner_Trend_D1", "VolTarget_Trend_D1", 
    "EMA_Pullback_H4", "Regime_Switcher_D1", "WEEKLY"
}

MR_STRATS = {
    "BB_MR_D1", "Keltner_MR_H4", "BB_MR"
}

BO_STRATS = {
    "Donchian_BO_D1", "BB_Squeeze_BO_D1"
}
# Variabili globali per dati caricati
_STRAT_ROSTER = {}
_UNIVERSE_DATA = {}
# Mapping strategia -> builder function
STRATEGY_BUILDERS = {
    # Trend strategies
    "EMA_Trend_D1": "build_signal_ema_trend_d1",
    "Keltner_Trend_D1": "build_signal_keltner_trend_d1", 
    "VolTarget_Trend_D1": "build_signal_voltarget_trend_d1",
    "EMA_Pullback_H4": "build_signal_ema_pullback_h4",
    "Regime_Switcher_D1": "build_signal_regime_switcher_d1",
    "WEEKLY": "build_signal_weekly",
    
    # Mean Reversion strategies
    "BB_MR_D1": "build_signal_bbmr_confirmed",
    "Keltner_MR_H4": "build_signal_keltner_mr_h4",
    "BB_MR": "build_signal_bbmr_confirmed",
    
    # Breakout strategies
    "Donchian_BO_D1": "build_signal_donchian_bo_d1",
    "BB_Squeeze_BO_D1": "build_signal_bb_squeeze_bo_d1"
}
...
def is_strategy_active_for(asset_label: str, strat_name: str) -> bool:
    """Verifica se una strategia è attiva per un asset nel roster"""
    if not _STRAT_ROSTER:
        return False
    active = set(_STRAT_ROSTER.get("per_asset", {}).get(asset_label, {}).get("active", []))
    return strat_name in active

def get_active_strategies_for_asset(asset_label: str) -> set:
    """Ritorna le strategie attive per un asset"""
    if not _STRAT_ROSTER:
        return set()
    return set(_STRAT_ROSTER.get("per_asset", {}).get(asset_label, {}).get("active", []))
...
# =============== NUOVI SIGNAL BUILDERS ===============

def build_signal_ema_trend_d1(item: dict) -> Optional[DeterministicSignal]:
    """Builder per strategia EMA_Trend_D1"""
    asset = item["asset"]
    symbol = item["vendorSymbol"]
    
    try:
        p = get_params("EMA_Trend_D1", asset)
        
        ema_fast = _to_int(p.get("ema_fast", 50), 50)
        ema_slow = _to_int(p.get("ema_slow", 200), 200)
        sl_mult = _to_float(p.get("sl_atr_mult", 2.0), 2.0)
        rr_tgt = _to_float(p.get("rr", 1.5), 1.5)
        
        d1 = fetch_ohlc_twelvedata(symbol, "1day", 260)
        if d1 is None or len(d1) < max(ema_slow + 10, 50):
            dbg_reason(asset, f"EMA_Trend_D1: dati D1 insufficienti")
            return None
            
        d1["ema_fast"] = ema(d1["close"], ema_fast)
        d1["ema_slow"] = ema(d1["close"], ema_slow)
        d1["atr14"] = atr_from_df(d1, 14)
        
        ema_f = float(d1["ema_fast"].iloc[-1])
        ema_s = float(d1["ema_slow"].iloc[-1])
        atr14 = float(d1["atr14"].iloc[-1])
        close = float(d1["close"].iloc[-1])
        
        # Trend direction
        direction = "LONG" if ema_f > ema_s else "SHORT"
        
        # Entry al prezzo corrente
        entry = close
        ...
        basis = {
            f"ema{ema_fast}_d1": round(ema_f, 5),
            f"ema{ema_slow}_d1": round(ema_s, 5),
            "atr14_d1": round(atr14, 5)
        }
        
        return DeterministicSignal(
            "EMA_Trend_D1", asset, symbol, direction, "D1",
            round(entry, 5), round(sl, 5), round(tp, 5), round(rr, 2),
            round(atr14, 5), basis
        )
        
    except Exception as e:
        dbg_reason(asset, f"EMA_Trend_D1: errore {e}")
        return None

def build_signal_keltner_trend_d1(item: dict) -> Optional[DeterministicSignal]:
    """Builder per strategia Keltner_Trend_D1"""
    asset = item["asset"]
    symbol = item["vendorSymbol"]
    
    try:
        p = get_params("Keltner_Trend_D1", asset)
        
        ema_period = _to_int(p.get("ema_period", 20), 20)
        atr_mult = _to_float(p.get("atr_mult", 2.0), 2.0)
        sl_mult = _to_float(p.get("sl_atr_mult", 2.5), 2.5)
        rr_tgt = _to_float(p.get("rr", 1.8), 1.8)
        
        d1 = fetch_ohlc_twelvedata(symbol, "1day", 260)
        if d1 is None or len(d1) < 50:
            dbg_reason(asset, f"Keltner_Trend_D1: dati D1 insufficienti")
            return None
            
        d1["ema"] = ema(d1["close"], ema_period)
        d1["atr"] = atr_from_df(d1, ema_period)
        
        ema_val = float(d1["ema"].iloc[-1])
        atr_val = float(d1["atr"].iloc[-1])
        close = float(d1["close"].iloc[-1])
        ...
        if close > upper:
            ...
            dbg_reason(asset, "Keltner_Trend_D1: prezzo dentro i canali")
            return None
            
        rr = abs((tp - entry) / (entry - sl)) if (entry - sl) != 0 else 0.0
        
        basis = {
            f"ema{ema_period}_d1": round(ema_val, 5),
            "keltner_upper": round(upper, 5),
            "keltner_lower": round(lower, 5),
            "atr_d1": round(atr_val, 5)
        }
        
        return DeterministicSignal(
            "Keltner_Trend_D1", asset, symbol, direction, "D1",
            round(entry, 5), round(sl, 5), round(tp, 5), round(rr, 2),
            round(atr_val, 5), basis
        )
        
    except Exception as e:
        dbg_reason(asset, f"Keltner_Trend_D1: errore {e}")
        return None

def build_signal_voltarget_trend_d1(item: dict) -> Optional[DeterministicSignal]:
    """Builder per strategia VolTarget_Trend_D1"""
    asset = item["asset"]
    symbol = item["vendorSymbol"]
    
    try:
        p = get_params("VolTarget_Trend_D1", asset)
        
        vol_target = _to_float(p.get("vol_target", 1.0), 1.0)
        ema_period = _to_int(p.get("ema_period", 21), 21)
        sl_mult = _to_float(p.get("sl_atr_mult", 2.0), 2.0)
        rr_tgt = _to_float(p.get("rr", 1.5), 1.5)
        
        d1 = fetch_ohlc_twelvedata(symbol, "1day", 260)
        if d1 is None or len(d1) < 50:
            dbg_reason(asset, f"VolTarget_Trend_D1: dati D1 insufficienti")
            return None
            
        d1["ema"] = ema(d1["close"], ema_period)
        d1["atr"] = atr_from_df(d1, 14)
        atrp = atr_percent(d1, 14)
        
        ema_val = float(d1["ema"].iloc[-1])
        atr_val = float(d1["atr"].iloc[-1])
        close = float(d1["close"].iloc[-1])
        atrp_val = float(atrp.iloc[-1])
        
        # Controllo volatilità target
        if atrp_val < vol_target * 0.8 or atrp_val > vol_target * 1.5:
            dbg_reason(asset, f"VolTarget_Trend_D1: ATR% {atrp_val:.2f} fuori target {vol_target}")
            return None
            
        # Trend direction
        direction = "LONG" if close > ema_val else "SHORT"
        entry = close
        ...
        basis = {
            f"ema{ema_period}_d1": round(ema_val, 5),
            "atr_pct": round(atrp_val, 3),
            "vol_target": vol_target,
            "atr14_d1": round(atr_val, 5)
        }
        
        return DeterministicSignal(
            "VolTarget_Trend_D1", asset, symbol, direction, "D1",
            round(entry, 5), round(sl, 5), round(tp, 5), round(rr, 2),
            round(atr_val, 5), basis
        )
        
    except Exception as e:
        dbg_reason(asset, f"VolTarget_Trend_D1: errore {e}")
        return None

def build_signal_ema_pullback_h4(item: dict) -> Optional[DeterministicSignal]:
    """Builder per strategia EMA_Pullback_H4"""
    asset = item["asset"]
    symbol = item["vendorSymbol"]
    
    try:
        p = get_params("EMA_Pullback_H4", asset)
        
        ema_fast = _to_int(p.get("ema_fast", 21), 21)
        ema_slow = _to_int(p.get("ema_slow", 50), 50)
        pullback_pct = _to_float(p.get("pullback_pct", 0.5), 0.5)
        sl_mult = _to_float(p.get("sl_atr_mult", 1.5), 1.5)
        rr_tgt = _to_float(p.get("rr", 2.0), 2.0)
        
        h4 = fetch_ohlc_twelvedata(symbol, "4h", 300)
        d1 = fetch_ohlc_twelvedata(symbol, "1day", 100)
        
        if h4 is None or d1 is None or len(h4) < 100 or len(d1) < 50:
            dbg_reason(asset, f"EMA_Pullback_H4: dati insufficienti")
            return None
            
        h4["ema_fast"] = ema(h4["close"], ema_fast)
        h4["ema_slow"] = ema(h4["close"], ema_slow)
        d1["atr14"] = atr_from_df(d1, 14)
        
        ema_f = float(h4["ema_fast"].iloc[-1])
        ema_s = float(h4["ema_slow"].iloc[-1])
        ...
        if ema_f > ema_s:
            direction = "LONG"
            # Cerca pullback verso EMA veloce
            if close < ema_f * (1 + pullback_pct/100):
                entry = close
                sl = entry - sl_mult * atr14
                r = entry - sl
                tp = entry + rr_tgt * r
            else:
                dbg_reason(asset, "EMA_Pullback_H4: nessun pullback LONG")
                return None
        else:
            direction = "SHORT"
            # Cerca pullback verso EMA veloce
            if close > ema_f * (1 - pullback_pct/100):
                ...
                dbg_reason(asset, "EMA_Pullback_H4: nessun pullback SHORT")
                return None
                
        rr = abs((tp - entry) / (entry - sl)) if (entry - sl) != 0 else 0.0
        
        basis = {
            f"ema{ema_fast}_h4": round(ema_f, 5),
            f"ema{ema_slow}_h4": round(ema_s, 5),
            "pullback_pct": pullback_pct,
            "atr14_d1": round(atr14, 5)
        }
        
        return DeterministicSignal(
            "EMA_Pullback_H4", asset, symbol, direction, "H4",
            round(entry, 5), round(sl, 5), round(tp, 5), round(rr, 2),
            round(atr14, 5), basis
        )
        
    except Exception as e:
        dbg_reason(asset, f"EMA_Pullback_H4: errore {e}")
        return None

def build_signal_keltner_mr_h4(item: dict) -> Optional[DeterministicSignal]:
    """Builder per strategia Keltner_MR_H4"""
    asset = item["asset"]
    symbol = item["vendorSymbol"]
    
    try:
        p = get_params("Keltner_MR_H4", asset)
        
        ema_period = _to_int(p.get("ema_period", 20), 20)
        atr_mult = _to_float(p.get("atr_mult", 2.0), 2.0)
        rsi_period = _to_int(p.get("rsi_period", 14), 14)
        rsi_buy = _to_float(p.get("rsi_buy", 30), 30)
        rsi_sell = _to_float(p.get("rsi_sell", 70), 70)
        sl_mult = _to_float(p.get("sl_atr_mult", 1.5), 1.5)
        rr_tgt = _to_float(p.get("rr", 1.5), 1.5)
        ...
        if h4 is None or d1 is None or len(h4) < 100 or len(d1) < 50:
            dbg_reason(asset, f"Keltner_MR_H4: dati insufficienti")
            return None
            
        h4["ema"] = ema(h4["close"], ema_period)
        h4["atr"] = atr_from_df(h4, ema_period)
        h4["rsi"] = rsi(h4["close"], rsi_period)
        d1["atr14"] = atr_from_df(d1, 14)
        
        ema_val = float(h4["ema"].iloc[-1])
        atr_val = float(h4["atr"].iloc[-1])
        rsi_val = float(h4["rsi"].iloc[-1])
        close = float(h4["close"].iloc[-1])
        atr14 = float(d1["atr14"].iloc[-1])
        ...
        if close <= lower and rsi_val <= rsi_buy:
            direction = "LONG"
            entry = close
            sl = entry - sl_mult * atr14
            tp = ema_val  # Target verso media
        elif close >= upper and rsi_val >= rsi_sell:
            direction = "SHORT"
            entry = close
            sl = entry + sl_mult * atr14
            tp = ema_val  # Target verso media
        else:
            dbg_reason(asset, f"Keltner_MR_H4: nessun segnale MR (RSI={rsi_val:.1f})")
            return None
            
        rr = abs((tp - entry) / (entry - sl)) if (entry - sl) != 0 else 0.0
        
        basis = {
            f"ema{ema_period}_h4": round(ema_val, 5),
            "keltner_upper": round(upper, 5),
            "keltner_lower": round(lower, 5),
            "rsi": round(rsi_val, 1),
            "atr14_d1": round(atr14, 5)
        ...
        return DeterministicSignal(
            "Keltner_MR_H4", asset, symbol, direction, "H4",
            round(entry, 5), round(sl, 5), round(tp, 5), round(rr, 2),
            round(atr14, 5), basis
        )
        
    except Exception as e:
        dbg_reason(asset, f"Keltner_MR_H4: errore {e}")
        return None

def build_signal_donchian_bo_d1(item: dict) -> Optional[DeterministicSignal]:
    """Builder per strategia Donchian_BO_D1"""
    asset = item["asset"]
    symbol = item["vendorSymbol"]
    
    try:
        p = get_params("Donchian_BO_D1", asset)
        
        period = _to_int(p.get("period", 20), 20)
        sl_mult = _to_float(p.get("sl_atr_mult", 2.0), 2.0)
        rr_tgt = _to_float(p.get("rr", 2.0), 2.0)
        
        d1 = fetch_ohlc_twelvedata(symbol, "1day", 260)
        if d1 is None or len(d1) < period + 20:
            dbg_reason(asset, f"Donchian_BO_D1: dati D1 insufficienti")
            return None
            
        d1["high_max"] = d1["high"].rolling(period).max()
        d1["low_min"] = d1["low"].rolling(period).min()
        d1["atr14"] = atr_from_df(d1, 14)
        
        high_max = float(d1["high_max"].iloc[-2])  # Precedente
        low_min = float(d1["low_min"].iloc[-2])   # Precedente
        close = float(d1["close"].iloc[-1])
        high = float(d1["high"].iloc[-1])
        low = float(d1["low"].iloc[-1])
        atr14 = float(d1["atr14"].iloc[-1])
        ...
        if high > high_max:
            ...
            dbg_reason(asset, f"Donchian_BO_D1: nessun breakout")
            return None
            
        rr = abs((tp - entry) / (entry - sl)) if (entry - sl) != 0 else 0.0
        
        basis = {
            f"donchian_{period}_high": round(high_max, 5),
            f"donchian_{period}_low": round(low_min, 5),
            "atr14_d1": round(atr14, 5)
        }
        
        return DeterministicSignal(
            "Donchian_BO_D1", asset, symbol, direction, "D1",
            round(entry, 5), round(sl, 5), round(tp, 5), round(rr, 2),
            round(atr14, 5), basis
        )
        
    except Exception as e:
        dbg_reason(asset, f"Donchian_BO_D1: errore {e}")
        return None

def build_signal_bb_squeeze_bo_d1(item: dict) -> Optional[DeterministicSignal]:
    """Builder per strategia BB_Squeeze_BO_D1"""
    asset = item["asset"]
    symbol = item["vendorSymbol"]
    
    try:
        p = get_params("BB_Squeeze_BO_D1", asset)
        
        bb_period = _to_int(p.get("bb_period", 20), 20)
        bb_std = _to_float(p.get("bb_std", 2.0), 2.0)
        squeeze_threshold = _to_float(p.get("squeeze_threshold", 0.02), 0.02)
        sl_mult = _to_float(p.get("sl_atr_mult", 2.0), 2.0)
        rr_tgt = _to_float(p.get("rr", 2.5), 2.5)
        
        d1 = fetch_ohlc_twelvedata(symbol, "1day", 260)
        if d1 is None or len(d1) < bb_period + 20:
            dbg_reason(asset, f"BB_Squeeze_BO_D1: dati D1 insufficienti")
            return None
            
        lo, ma, up = bbands(d1["close"], bb_period, bb_std)
        d1["atr14"] = atr_from_df(d1, 14)
        
        # Bollinger Band Width
        bbw = (up - lo) / ma
        bbw_current = float(bbw.iloc[-1])
        bbw_avg = float(bbw.rolling(50).mean().iloc[-1])
        
        close = float(d1["close"].iloc[-1])
        ma_val = float(ma.iloc[-1])
        atr14 = float(d1["atr14"].iloc[-1])
        
        # Squeeze condition: BBW molto bassa
        if bbw_current > squeeze_threshold:
            dbg_reason(asset, f"BB_Squeeze_BO_D1: nessuna compressione (BBW={bbw_current:.4f})")
            return None
        ...
        basis = {
            f"bb_ma_{bb_period}": round(ma_val, 5),
            "bbw_current": round(bbw_current, 4),
            "bbw_avg": round(bbw_avg, 4),
            "squeeze_threshold": squeeze_threshold,
            "atr14_d1": round(atr14, 5)
        }
        
        return DeterministicSignal(
            "BB_Squeeze_BO_D1", asset, symbol, direction, "D1",
            round(entry, 5), round(sl, 5), round(tp, 5), round(rr, 2),
            round(atr14, 5), basis
        )
        
    except Exception as e:
        dbg_reason(asset, f"BB_Squeeze_BO_D1: errore {e}")
        return None

def build_signal_regime_switcher_d1(item: dict) -> Optional[DeterministicSignal]:
    """Builder per strategia Regime_Switcher_D1"""
    asset = item["asset"]
    symbol = item["vendorSymbol"]
    regime = item.get("regime", "INDEFINITO")
    
    try:
        p = get_params("Regime_Switcher_D1", asset)
        
        # Parametri adattivi basati sul regime
        if regime == "TRENDING":
            return build_signal_weekly(item)  # Usa logica trend
        elif regime in ("LATERALE", "LATERALE (Compressione)"):
            return build_signal_bbmr_confirmed(item)  # Usa logica MR
        else:
            dbg_reason(asset, f"Regime_Switcher_D1: regime {regime} non supportato")
            return None
            
    except Exception as e:
        dbg_reason(asset, f"Regime_Switcher_D1: errore {e}")
        return None

def build_signals_for_item(item: dict) -> List[DeterministicSignal]:
    """
    Genera segnali per un asset usando TUTTE le strategie attive nel roster
    """
    asset = item.get("asset", "N/A")
    regime = (item.get("regime") or "INDEFINITO").upper()
    signals: List[DeterministicSignal] = []
    
    # Ottieni strategie attive per questo asset dal roster
    active_strategies = get_active_strategies_for_asset(asset)
    
    if not active_strategies:
        dbg_reason(asset, "Nessuna strategia attiva nel roster")
        return signals
    
    # Ottieni strategie consentite da Universe
    universe_strategies = set(item.get("strategies", []))
    
    # Mappa nomi Universe (MAIUSCOLO) -> Roster (CamelCase)
    STRAT_NAME_MAP = {
        "WEEKLY": "WEEKLY",
        "BB_MR": "BB_MR_D1",
        "BB_MR_D1": "BB_MR_D1",
        "KELTNER_MR_H4": "Keltner_MR_H4",
        "REGIME_SWITCHER_D1": "Regime_Switcher_D1",
        "DONCHIAN_BO_D1": "Donchian_BO_D1",
        "KELTNER_TREND_D1": "Keltner_Trend_D1",
        "EMA_TREND_D1": "EMA_Trend_D1",
        "EMA_PULLBACK_H4": "EMA_Pullback_H4",
        "VOLTARGET_TREND_D1": "VolTarget_Trend_D1",
        "BB_SQUEEZE_BO_D1": "BB_Squeeze_BO_D1"
    }
    
    # Converti strategie Universe in CamelCase
    universe_camel = {STRAT_NAME_MAP.get(s.upper(), s) for s in universe_strategies}
    
    # Intersezione: strategie attive nel roster E consentite da Universe
    allowed_strategies = active_strategies & universe_camel
    
    if not allowed_strategies:
        dbg_reason(asset, f"Nessuna intersezione Universe∩Roster: active={active_strategies}, universe={universe_camel}")
        return signals
    
    # Determina strategie candidate basate sul regime
    candidates = set()
    
    if regime == "TRENDING":
        candidates = allowed_strategies & TREND_STRATS
    elif regime in ("LATERALE", "LATERALE (COMPRESSIONE)"):
        candidates = allowed_strategies & MR_STRATS
    elif regime in ("VOLATILE", "COMPRESSIONE FORTE"):
        candidates = allowed_strategies & BO_STRATS
    else:
        # Per regimi indefiniti, prova tutte le strategie consentite
        candidates = allowed_strategies
    
    dbg(f"{asset}: regime={regime}, active={active_strategies}, universe={universe_camel}, candidates={candidates}")
    
    # Genera segnali per ogni strategia candidata
    for strategy_name in candidates:
        builder_name = STRATEGY_BUILDERS.get(strategy_name)
        if not builder_name:
            dbg_reason(asset, f"Builder non trovato per strategia {strategy_name}")
            continue
            
        try:
            # Ottieni la funzione builder
            builder_func = globals().get(builder_name)
            if not builder_func:
                dbg_reason(asset, f"Funzione {builder_name} non implementata")
                continue
                
            # Genera il segnale
            signal = builder_func(item)
            if signal:
                dbg(f"{asset}: segnale generato da {strategy_name}")
                signals.append(signal)
            else:
                dbg(f"{asset}: nessun segnale da {strategy_name}")
                
        except Exception as e:
            dbg_reason(asset, f"Errore in {strategy_name}: {e}")
            continue
...
def main():
    ...
    try:
        ...
        _UNIVERSE_DATA = _load_json(UNIVERSE_FILE, {})
        ...
        for item in tradables:
            ...
            try:
                ...
                # Arricchisci item con strategie Universe
                if _UNIVERSE_DATA and "allow" in _UNIVERSE_DATA:
                    item["strategies"] = _UNIVERSE_DATA["allow"].get(asset, [])
                else:
                    item["strategies"] = []
