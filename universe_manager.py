...
def pick_universe(evals: List[Dict[str,Any]], top_n: int = TOP_N) -> Dict[str,Any]:
    ...
    for e in sel:
        strat = []
        # Aggiungi tutte le strategie disponibili se i punteggi sono buoni
        if e["weekly"]["score"] >= 2: 
            strat.extend(["WEEKLY", "EMA_Trend_D1", "Keltner_Trend_D1", "VolTarget_Trend_D1", "EMA_Pullback_H4", "Regime_Switcher_D1"])
        if e["bbmr"]["score"] >= 2: 
            strat.extend(["BB_MR", "BB_MR_D1", "Keltner_MR_H4"])
        # Aggiungi strategie breakout se entrambi i punteggi sono decenti
        if e["weekly"]["score"] >= 1 and e["bbmr"]["score"] >= 1:
            strat.extend(["Donchian_BO_D1", "BB_Squeeze_BO_D1"])
