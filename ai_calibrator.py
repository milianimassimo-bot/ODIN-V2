...
AUTO_GATE_STRATS = {s.strip() for s in os.getenv("AUTO_GATE_STRATS", "BB_MR_D1,WEEKLY,EMA_Trend_D1,Keltner_Trend_D1,VolTarget_Trend_D1,EMA_Pullback_H4,Keltner_MR_H4,Donchian_BO_D1,BB_Squeeze_BO_D1,Regime_Switcher_D1").split(",") if s.strip()}
...
def _load_retrain_text(p: str) -> dict:
    ...
    # Pattern più flessibile per tutte le strategie
    with open(p, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            # Cerca pattern: ASSET: ... STRATEGY[stats] ... | STRATEGY[stats] ...
            if not re.search(r"^[A-Z/]+:", line):
                continue
                
            asset_match = re.match(r"^([A-Z/]+):", line)
            if not asset_match:
                continue
            asset = asset_match.group(1).strip()
            
            # Estrai tutte le strategie con pattern STRATEGY[stats]
            strat_data = {}
            for strat in AUTO_GATE_STRATS:
                pattern = rf"{re.escape(strat)}$(.*?)$"
                match = re.search(pattern, line)
                if match:
                    strat_data[strat] = _parse_block_stats(match.group(1))
            
            if strat_data:
                out[asset] = strat_data
...
def _load_retrain_json(p: str) -> dict:
    ...
    for asset, stratmap in raw.items():
        ...
        # Supporta tutte le strategie dinamicamente
        asset_data = {}
        for strat in AUTO_GATE_STRATS:
            if strat in stratmap:
                asset_data[strat] = norm(stratmap.get(strat))
        
        if asset_data:  # Solo se ha almeno una strategia
            out[asset] = asset_data
